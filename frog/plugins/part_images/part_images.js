						...
						success: function( content )
						{
							...
							$dialog[part_id] = $(content).dialog({
								...
								buttons: [
								{
									...
									click: function()
									{
										$(this).find('#'+part_id+'-ai-upload-file').uploadifyUpload();
									}
								},
								{
									text: 'Close',
							...
							var onAllComplete_handler = function( event, data )
							{
								...
								$dialog[part_id].dialog('close');
								
							};
							//alert($dialog[part_id].find('#'+part_id+'-ai-upload-file').attr('id'))
							// Uploadify
							$dialog[part_id].find('#'+part_id+'-ai-upload-file').uploadify({
								// options
								uploader  : PLUGINS_URL + 'files/uploadify/uploadify.swf',
								script    : $dialog[part_id].find('#'+part_id+'-ai-upload-form').attr('action'),
