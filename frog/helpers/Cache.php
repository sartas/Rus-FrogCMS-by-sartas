...
abstract class Cache {
	 ...
	 */
	public static function simple( $name, $data = NULL, $lifetime = null )
	{
		// Cache file is a hash of the name
		$file = sha1( $name ) . '.txt';

		// Cache directories are split by keys to prevent filesystem overload
		$dir = FROG_ROOT . DIRECTORY_SEPARATOR . Cache::CACHE_DIR . DIRECTORY_SEPARATOR . $file[0] . $file[1] . DIRECTORY_SEPARATOR;
	...
	public static function instance( $group = NULL )
	{
		...
		/*
		  ...
		  {
		  throw new framework_exception_handler('Failed to load Kohana Cache group: :group', array(':group' => $group));
		  }
		...
		use_helper( 'cache/' . $group );
		$cache_class = 'Cache' . ucfirst( $group );
		Cache::$instances[$group] = new $cache_class();
...
function content_cache_page_requested()
{
	if ( FROG_BACKEND )
		return;
	//$cached_page = Cache::instance()->get( ltrim( CURRENT_URI, '/' ) );
	$cached_page = Cache::simple( ltrim( CURRENT_URI, '/' ) );
...
function content_cache_page_found( $page )
{
	ob_start();
	$page->display();
	echo $content = ob_get_clean();

	//Cache::instance()->set( $page->url, $content );
	Cache::simple( $page->url, $content );

	if ( DELETE_PAGE_CACHE_AFTER_EDIT )
	{
		$connection = Record::getConnection();
		$sql = 'INSERT INTO ' . TABLE_PREFIX . 'cached_pages (page_id, url) VALUES (?, ?)';
		$stmt = $connection->prepare( $sql );
		$stmt->execute( array($page->id, $page->url) );
	}

	exit();
}

function content_cache_page_delete( $page )
{
	if ( DELETE_PAGE_CACHE_AFTER_EDIT )
	{
		$connection = Record::getConnection();
		$sql = 'SELECT * FROM ' . TABLE_PREFIX . 'cached_pages WHERE page_id = ?';
		$stmt = $connection->prepare( $sql );
	...
	if ( $page = $stmt->fetchObject() )
	{
		...
		//Cache::instance()->delete( $page->url );
		Cache::simple( $page->url, null, 0 );

		if ( DELETE_PAGE_CACHE_AFTER_EDIT )
		{
			$sql = 'DELETE FROM ' . TABLE_PREFIX . 'cached_pages WHERE id = ?';
			$stmt = $connection->prepare( $sql );
			$stmt->execute( array($page->id) );
		}
	}
...
if ( USE_PAGE_CACHE )
{
	Observer::observe( 'page_found', 'content_cache_page_found' );

	content_cache_page_requested();

	if ( DELETE_PAGE_CACHE_AFTER_EDIT )
	{
		Observer::observe( 'page_edit_after_save', 'content_cache_page_delete' );
	}
}
?>
