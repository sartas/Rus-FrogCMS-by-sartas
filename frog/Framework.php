...
// clear
	...
	public static function json( $var, $value=null )
	{
...
function get_url()
{
	...
	if ( count( $params ) === 0 )
		return BASE_URL;
	if ( count( $params ) === 1 )
		return BASE_URL . $params[0] . (FROG_BACKEND ? '' : URL_SUFFIX);
	...
	return BASE_URL . preg_replace( '/^\/(.*)$/', '$1', $url ) . (FROG_BACKEND ? '' : URL_SUFFIX);
}
...
function remove_array_xss( $ar )
{
	...
	{
		if ( is_array( $k ) )
			$k = remove_array_xss( $k );
		else
			$k = remove_xss( $k );

		if ( is_array( $v ) )
			$v = remove_array_xss( $v );
		else
			$v = remove_xss( $v );
	...
	while ( list($k, $v) = each( $in ) )
	{
		foreach ( $v as $key => $val )
		{
			...
			if ( !is_array( $val ) )
				...
				$val = remove_array_xss( $val );

			if ( !is_array( $key ) )
				$key = remove_xss( $key );
			else
				$key = remove_array_xss( $key );
 ...
 */
function page_not_found( $uri = CURRENT_URI )
{
	Observer::notify( 'page_not_found', $uri );

	$message = '<h1>Content Not Found (404)</h1>
		<p>The content you requested was not found. It may have been deleted or you may have entered an incorrect address.</p>
		<p>Please return to the <a href="' . URL_PUBLIC . '">home page</a> to view the main navigation.</p>';

	die_info( $message, 404 );
	exit;
}

function die_info( $message, $http_status = false )
{
	if ( $http_status )
	{
		switch ( $http_status )
	...
	echo new View( 'info', array('message' => $message, 'http_status' => $http_status) );
	exit;
}
