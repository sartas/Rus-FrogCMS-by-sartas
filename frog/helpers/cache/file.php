...
class CacheFile extends Cache {
	 ...
	 *     $filename = CacheFile::filename($this->_sanitize_id($id));
	 *
...
//			$directory = Arr::get($this->_config, 'cache_dir', Kohana::$cache_dir);
			$directory = FROG_ROOT . '/' . CacheFile::CACHE_DIR;
		...
		{
			throw new Exception( 'Unable to create cache directory as a file already exists : :resource', array(':resource' => $this->_cache_dir->getRealPath()) );
		}
		...
		{
			throw new Exception( 'Unable to write to the cache directory :resource', array(':resource' => $this->_cache_dir->getRealPath()) );
		}
	}
