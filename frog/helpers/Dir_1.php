...
abstract class DirAndFiles {
	...
	{
		//	return $this->path();
		return rename( $this->path(), $this->getDirPath() . '/' . $new_name );
	}
	...
	/*
	  ...
	  {
	  return substr( $this->path(), strpos( $this->path(), $dir_name ), strlen( $this->path() ) );
	  }
	 */
	...
	{
		return substr( $this->path(), strrpos( $this->path(), '/' ) + 1, strlen( $this->path() ) );
	}
...
class Dir extends DirAndFiles {
	...
	public function getDirs()
	{
		...
		{
			$dirs[] = new Dir( $this->path() . '/' . $dir_name );
		}
	...
	public function getFiles()
	{
		...
		foreach ( $this->files() as $file_name )
		{
			...
			$files[] = new $class_name( $this->path() . '/' . $file_name );
		}
	...
	public function hasChildrens( $only_dirs = false )
	{
		if ( $only_dirs )
			...
			return!(empty( $this->dirs() ) && empty( $this->files() ));
	}
...
class DirFileImage extends DirFile {
	...
	public function save( $filename=null, $image_type=IMAGETYPE_JPEG, $compression=75, $permissions=null )
	{
		...
		if ( $filename == null )
		{
			$filename = $this->getDirPath() . $this->getNewWidth() . 'x' . $this->getNewHeight() . '-' . $this->name();
		}
	...
	{
		$Dir = new Dir( $this->getDirPath() );

		foreach ( $Dir->getFiles() as $file )
		{
			if ( $this->endsWith( $file, $this->name() ) )
			{
				$file->remove();
			}
		}
